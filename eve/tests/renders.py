...
class TestRenders(TestBase):
    async def test_default_render(self):
        r = await self.test_client.get("/")
        self.assertEqual(r.content_type, "application/json")

    async def test_json_render(self):
        r = await self.test_client.get("/", headers=[("Accept", "application/json")])
        self.assertEqual(r.content_type, "application/json")

    async def test_xml_render(self):
        r = await self.test_client.get("/", headers=[("Accept", "application/xml")])
        self.assertTrue("application/xml" in r.content_type)

    async def test_xml_url_escaping(self):
        r = await self.test_client.get(
            "%s?max_results=1" % self.known_resource_url,
            headers=[("Accept", "application/xml")],
        )
        self.assertTrue(b"&amp;" in (await r.get_data()))

    async def test_xml_leaf_escaping(self):
        ...
        self.assertTrue(b"12345 &amp; 6789" in (await r.get_data()))

    async def test_xml_ordered_nodes(self):
    ...
    async def test_xml_data_relation_hateoas(self):
        # We need to assign a `person` to our test invoice
        _db = self.connection[MONGO_DBNAME]
        ...
        self.assertTrue(data_relation_opening_tag in (await r.get_data()).decode())

    async def test_unknown_render(self):
        r = await self.test_client.get("/", headers=[("Accept", "application/html")])
        self.assertEqual(r.content_type, "application/json")

    async def test_json_xml_disabled(self):
    ...
    async def test_json_disabled(self):
    ...
    async def test_xml_disabled(self):
    ...
    async def test_json_keys_sorted(self):
        self.app.config["JSON_SORT_KEYS"] = True
        r = await self.test_client.get(
            self.known_resource_url, headers=[("Accept", "application/json")]
        )
        request_data = await r.get_data()
        self.assertEqual(
            json.dumps(json.loads(request_data), sort_keys=True).encode(), request_data
        )

    async def test_jsonp_enabled(self):
        arg = "callback"
        self.app.config["JSONP_ARGUMENT"] = arg
        val = "JSON_CALLBACK"
        r = await self.test_client.get("/?%s=%s" % (arg, val))
        self.assertTrue((await r.get_data()).decode("utf-8").startswith(val))
    ...
    async def test_CORS(self):
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://example.com")])
        self.assert200(r.status_code)
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://1of2.com")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "http://1of2.com")

        r = await self.test_client.get("/", headers=[("Origin", "http://2of2.com")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "http://2of2.com")

        r = await self.test_client.get("/", headers=[("Origin", "http://notreally.com")])
        self.assert200(r.status_code)
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://wwwxgithub.com")])
        self.assert200(r.status_code)
        self.assertFalse(
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://1of2.com:8000")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "")

    async def test_CORS_regex(self):
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://sub-123.domain.com")])
        self.assert200(r.status_code)
        self.assertEqual(
        ...
        r = await self.test_client.get("/", headers=[("Origin", "http://sub-123xdomain.com")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "")

        # test that invalid regexes are ignored, especially '*'
        self.app.config["X_DOMAINS_RE"] = ["*"]
        r = await self.test_client.get("/", headers=[("Origin", "http://www.example.com")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "")
    ...
    async def test_CORS_OPTIONS(self, url="/", methods=None):
        if methods is None:
            methods = []

        r = await self.test_client.open(url, method="OPTIONS")
        self.assertFalse("Access-Control-Allow-Origin" in r.headers)
        self.assertFalse("Access-Control-Allow-Methods" in r.headers)
        self.assertFalse("Access-Control-Max-Age" in r.headers)
        self.assertFalse("Access-Control-Expose-Headers" in r.headers)
        ...
        r = await self.test_client.get(url, headers=[("Origin", "http://not_an_example.com")])
        self.assert200(r.status_code)
        self.assertEqual(r.headers["Access-Control-Allow-Origin"], "")
        for m in methods:
            self.assertTrue(m in r.headers["Access-Control-Allow-Methods"])

    async def test_CORS_OPTIONS_resources(self):
        prefix = api_prefix(
        ...
        for _, settings in self.app.config["DOMAIN"].items():
            ...
            await self.test_CORS_OPTIONS(url, methods)

    async def test_CORS_OPTIONS_item(self):
        prefix = api_prefix(
            self.app.config["URL_PREFIX"], self.app.config["API_VERSION"]
        )
