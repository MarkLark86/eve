...
class TestBasicAuth(TestBase):
    ...
    async def test_restricted_home_access(self):
        r = await self.test_client.get("/")
        self.assert401(r.status_code)

    async def test_restricted_resource_access(self):
        r = await self.test_client.get(self.known_resource_url)
        self.assert401(r.status_code)
        r = await self.test_client.post(self.known_resource_url)
        self.assert401(r.status_code)
        r = await self.test_client.delete(self.known_resource_url)
        self.assert401(r.status_code)

    async def test_restricted_item_access(self):
        r = await self.test_client.get(self.item_id_url)
        self.assert401(r.status_code)
        r = await self.test_client.patch(self.item_id_url)
        self.assert401(r.status_code)
        r = await self.test_client.delete(self.item_id_url)
        self.assert401(r.status_code)

    async def test_authorized_home_access(self):
        r = await self.test_client.get("/", headers=self.valid_auth)
        self.assert200(r.status_code)

    async def test_authorized_resource_access(self):
        r = await self.test_client.get(self.known_resource_url, headers=self.valid_auth)
        self.assert200(r.status_code)
        ...
        r = await self.test_client.delete(self.known_resource_url, headers=self.valid_auth)
        self.assert204(r.status_code)

    async def test_authorized_item_access(self):
        r = await self.test_client.get(self.item_id_url, headers=self.valid_auth)
        self.assert200(r.status_code)
        r = await self.test_client.patch(
            self.item_id_url, json={"k": "value"}, headers=self.valid_auth
        )
        self.assert428(r.status_code)
        r = await self.test_client.delete(self.item_id_url, headers=self.valid_auth)
        self.assert428(r.status_code)

    async def test_authorized_media_access(self):
        ...
        r, s = await self.parse_response(
            await self.test_client.post(
                ...
                headers=self.valid_media_auth
            )
        )
        self.assert201(s)

        file_url = r["media"]
        r = await self.test_client.get(file_url, headers=self.invalid_auth)
        self.assert401(r.status_code)
        r = await self.test_client.get(file_url, headers=self.valid_auth)
        self.assert200(r.status_code)

    async def test_authorized_schema_access(self):
    ...
    async def test_unauthorized_home_access(self):
        r = await self.test_client.get("/", headers=self.invalid_auth)
        self.assert401(r.status_code)

    async def test_unauthorized_resource_access(self):
        r = await self.test_client.get(self.known_resource_url, headers=self.invalid_auth)
        self.assert401(r.status_code)
        r = await self.test_client.post(self.known_resource_url, headers=self.invalid_auth)
        self.assert401(r.status_code)
        r = await self.test_client.delete(self.known_resource_url, headers=self.invalid_auth)
        self.assert401(r.status_code)

    async def test_unauthorized_item_access(self):
        r = await self.test_client.get(self.item_id_url, headers=self.invalid_auth)
        self.assert401(r.status_code)
        r = await self.test_client.patch(self.item_id_url, headers=self.invalid_auth)
        self.assert401(r.status_code)
        r = await self.test_client.delete(self.item_id_url, headers=self.invalid_auth)
        self.assert401(r.status_code)

    async def test_unauthorized_schema_access(self):
        self.app.config["SCHEMA_ENDPOINT"] = "schema"
        self.app._init_schema_endpoint()
    ...
    async def test_home_public_methods(self):
        self.app.config["PUBLIC_METHODS"] = ["GET"]
        r = await self.test_client.get("/")
        self.assert200(r.status_code)
        await self.test_restricted_resource_access()
        await self.test_restricted_item_access()

    async def test_public_methods_resource(self):
        self.app.config["PUBLIC_METHODS"] = ["GET"]
        ...
        for resource in domain:
            ...
            self.assert200(r.status_code)
            r = await self.test_client.post(url, json={"key1": "value1"})
            self.assert401or405(r.status_code)
            r = await self.test_client.delete(url)
            self.assert401or405(r.status_code)
        await self.test_restricted_item_access()

    async def test_public_methods_but_locked_resource(self):
        self.app.config["PUBLIC_METHODS"] = ["GET"]
        domain = self.app.config["DOMAIN"]
    ...
    async def test_public_methods_but_locked_item(self):
    ...
    async def test_public_methods_item(self):
        self.app.config["PUBLIC_ITEM_METHODS"] = ["GET"]
    ...
    async def test_bad_auth_class(self):
    ...
    async def test_instanced_auth(self):
    ...
    async def test_rfc2617_response(self):
        r = await self.test_client.get("/")
        self.assert401(r.status_code)
        self.assertTrue(
    ...
    async def test_allowed_roles_does_not_change(self):
        await self.test_client.get(self.known_resource_url)
        resource = self.app.config["DOMAIN"][self.known_resource]
        self.assertEqual(resource["allowed_roles"], ["admin"])

    async def test_allowed_item_roles_does_not_change(self):
        await self.test_client.get(self.item_id_url)
        resource = self.app.config["DOMAIN"][self.known_resource]
...
class TestHMACAuth(TestBasicAuth):
    ...
    async def test_post_resource_hmac_auth(self):
        ...
        r, status = await self.parse_response(
            await self.app.test_client().get(url, headers=self.valid_auth)
        )
...
class TestResourceAuth(TestBase):
    async def test_resource_only_auth(self):
        ...
        self.assert401(r.status_code)
        # both with BasicAuth.
        _, status = await self.parse_response(
            await self.test_client.get(self.known_resource_url, headers=basic_auth)
        )
        self.assert200(status)
        _, status = await self.parse_response(
            await self.test_client.get(self.item_id_url, headers=basic_auth)
        )
        self.assert200(status)

        # 'empty' resource endpoint is also protected
        r = await self.test_client.get(self.empty_resource_url)
        self.assert401(r.status_code)
        # but with TokenAuth
        r = await self.test_client.get(self.empty_resource_url, headers=token_auth)
        self.assert200(r.status_code)

        # other resources are not protected
        r = await self.test_client.get(self.readonly_resource_url)
        self.assert200(r.status_code)
...
class TestUserRestrictedAccess(TestBase):
    async def asyncSetUp(self):
        ...
        self.data = {"ref": "0123456789123456789012345"}
    ...
    async def test_get(self):
        data, status = await self.parse_response(
            await self.test_client.get(self.url, headers=self.valid_auth)
        )
        self.assert200(status)
        ...
        await _db.contacts.insert_one(new_user)

        # Verify that we can retrieve it
        data2, status2 = await self.parse_response(
            await self.test_client.get(self.url, headers=self.valid_auth)
        )
        self.assert200(status2)
        self.assertEqual(len(data2["_items"]), 1)

    async def test_get_by_auth_field_criteria(self):
        ...
        """
        _, status = await self.parse_response(
            await self.test_client.get(self.user_username_url, headers=self.valid_auth)
        )
        self.assert401(status)

    async def test_get_by_auth_field_id(self):
        """To test handling of ObjectIds"""
        # set auth_field to `_id`
        self.domain["users"][self.field_name] = self.domain["users"]["id_field"]

        _, status = await self.parse_response(
            await self.test_client.get(self.user_id_url, headers=self.valid_auth)
        )
        self.assert401(status)

    async def test_filter_by_auth_field_id(self):
    ...
    async def test_unique_to_user_on_post(self):
        ...
        r = await self.test_client.post(
            self.url, json=self.data, headers=alt_auth
        )
    ...
    async def test_post_resource_auth(self):
        # Ticket #231.
        ...
        _, status = await self.parse_response(r)

        # Verify that we can retrieve the same document
        data, status = await self.parse_response(
            await self.app.test_client().get(self.url, headers=self.valid_auth)
        )
        self.assert200(status)
        self.assertEqual(len(data["_items"]), 1)
        self.assertEqual(data["_items"][0]["ref"], self.data["ref"])

    async def test_post_bandwidth_saver_off_resource_auth(self):
        ...
        r, status = await self.parse_response(r)
    ...
    async def test_put(self):
        ...
        # post document
        data, status = await self.post()

        # retrieve document metadata
        url = "%s/%s" % (self.url, data["_id"])
        response = await self.test_client.get(url, headers=self.valid_auth)
        ...
        data, status = await self.parse_response(
            await self.test_client.get(url, headers=self.valid_auth)
        )
        self.assert200(status)
    ...
    async def test_put_resource_auth(self):
        # no global auth.
        self.app = Eve(settings=self.settings_file)
        await self.app.init_resources()
        ...
        # retrieve document metadata
        url = "%s/%s" % (self.url, data["_id"])
        response = await self.app.test_client().get(url, headers=self.valid_auth)
        etag = response.headers["ETag"]
        ...
        headers = [("If-Match", etag), self.valid_auth[0]]
        response, status = await self.parse_response(
            await self.app.test_client().put(
                url,
                json=changes,
                headers=headers,
            )
    ...
    async def test_put_bandwidth_saver_off_resource_auth(self):
        """Test that when BANDWIDTH_SAVER is turned off the auth_field is
        not exposed in the response payload
        """
        self.app.config["BANDWIDTH_SAVER"] = False
    ...
    async def test_delete(self):
        ...
        await _db.contacts.find().to_list(None)
        docs_num = await _db.contacts.count_documents({})
        ...
        response, status = await self.parse_response(
            await self.test_client.delete(self.url, headers=self.valid_auth)
        )
        ...
        self.assertEqual(await _db.contacts.count_documents({}), docs_num)
    ...
    async def test_delete_item(self):
        ...
        # delete the document
        response, status = await self.parse_response(
            await self.test_client.delete(url, headers=headers)
        )
        self.assert204(status)
    ...
    async def test_delete_item_soft_delete_enabled(self):
        ...
        # make sure no other document has been deleted.
        self.assertEqual(
            await _db.contacts.count_documents({"_deleted": {"$ne": True}}), docs_num
        )
        self.assertEqual(await _db.contacts.count_documents({"_deleted": True}), 1)

        challenge = await _db.contacts.find_one({"_deleted": True})
