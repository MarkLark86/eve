...
class TestPost(TestBase):
    async def test_unknown_resource(self):
        _, status = await self.post(self.unknown_resource_url, data={})
        self.assert404(status)

    async def test_readonly_resource(self):
        _, status = await self.post(self.readonly_resource_url, data={})
        self.assert405(status)

    async def test_post_to_item_endpoint(self):
        _, status = await self.post(self.item_id_url, data={})
        self.assert405(status)

    async def test_validation_error(self):
        r, status = await self.post(self.known_resource_url, data={"ref": "123"})
        self.assertValidationErrorStatus(status)
        self.assertValidationError(r, {"ref": "min length is 25"})

        r, status = await self.post(self.known_resource_url, data={"prog": 123})
        self.assertValidationErrorStatus(status)
        self.assertValidationError(r, {"ref": "required"})

    async def test_post_bulk_insert_on_disabled_bulk(self):
        r, status = await self.post(
            self.disabled_bulk_url,
            data=[{"string_field": "123"}, {"string_field": "123"}],
        )
        self.assert400(status)

    async def test_post_empty_bulk_insert(self):
        r, status = await self.post(self.empty_resource_url, data=[])
        self.assert400(status)

    async def test_post_empty_resource(self):
        data = []
        for _ in range(10):
    ...
    async def test_post_string(self):
        test_field = "ref"
        test_value = "1234567890123456789054321"
        data = {test_field: test_value}
        await self.assertPostItem(data, test_field, test_value)

    async def test_post_duplicate_key(self):
        data = {"ref": "1234567890123456789054321"}
        r = await self.perform_post(data)
        id_field = self.domain[self.known_resource]["id_field"]
        item_id = r[id_field]
        data = {"ref": "0123456789012345678901234", id_field: item_id}
        r, status = await self.post(self.known_resource_url, data=data)
        self.assertEqual(status, 409)

    async def test_post_integer(self):
    ...
    async def test_post_list_as_array(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        test_field = "role"
    ...
    async def test_post_integer_zero(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
    ...
    async def test_post_float_zero(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        test_field = "afloat"
        test_value = 0.0
        data = {test_field: test_value}
    ...
    async def test_post_datetime(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        test_field = "born"
    ...
    async def test_post_objectid(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        test_field = "tid"
    ...
    async def test_post_null_objectid(self):
    ...
    async def test_post_default_value(self):
    ...
    async def test_post_default_value_none(self):
        ...
        await self.assertPostItem(data, "title", "")

        title["type"] = "integer"
        title["default"] = 0
        self.app.set_defaults()
        data = {"ref": "TTTTTTTTTTTTTTTTTTTTTTTTT"}
        await self.assertPostItem(data, "title", 0)

        title["type"] = "boolean"
        title["default"] = False
        self.app.set_defaults()
        data = {"ref": "QQQQQQQQQQQQQQQQQQQQQQQQQ"}
        await self.assertPostItem(data, "title", False)

    async def test_multi_post_valid(self):
        data = [
        ...
        async with self.app.test_request_context(path="/"):
            ...
            r = await contacts.count_documents({"ref": "9234567890123456789054321"})
            self.assertTrue(r == 1)
            r = await contacts.count_documents({"ref": "5432112345678901234567890"})
            self.assertTrue(r == 1)

    async def test_multi_post_invalid(self):
        data = [
            {"ref": "9234567890123456789054321"},
            {"prog": 9999},
            {"ref": "5432112345678901234567890", "role": ["agent"]},
        ...
        async with self.app.test_request_context(path="/"):
            contacts = self.app.data.driver.db["contacts"]
            r = await contacts.count_documents({"prog": 9999})
            self.assertTrue(r == 0)
            r = await contacts.count_documents({"ref": "9234567890123456789054321"})
            self.assertTrue(r == 0)

    async def test_post_x_www_form_urlencoded(self):
        test_field = "ref"
        test_value = "1234567890123456789054321"
        data = {test_field: test_value}
        r, status = await self.parse_response(
            await self.test_client.post(self.known_resource_url, json=data)
        )
        self.assert201(status)
        self.assertTrue("OK" in r[STATUS])
        self.assertPostResponse(r)

    async def test_post_x_www_form_urlencoded_number_serialization(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        test_field = "anumber"
        test_value = 34
        data = {test_field: test_value}
    ...
    async def test_post_auto_collapse_multiple_keys(self):
        ...
        r, status = await self.parse_response(resp)
        self.assert201(status)
    ...
    async def test_post_auto_collapse_media_list(self):
        self.app.config["AUTO_COLLAPSE_MULTI_KEYS"] = True
        await self.app.register_resource(
            "test_res",
        ...
        obj = await _db.test_res.find_one({id_field: ObjectId(r[id_field])})
        media_ids = obj["list_field"]
        self.assertEqual(len(media_ids), 2)
        async with self.app.test_request_context(path="/"):
            for i in [0, 1]:
                self.assertTrue(await self.app.media.exists(media_ids[i], "test_res"))
        ...
        async with self.app.test_request_context(path="/"):
            for i in [0, 1]:
                self.assertFalse(await self.app.media.exists(media_ids[i], "test_res"))

    async def test_post_auto_create_lists(self):
    ...
    async def test_post_decimal_number_success(self):
        data = {"decimal_number": 100}
        r, status = await self.post("/invoices/", data=data)
        self.assert201(status)
        self.assertPostResponse(r)
        id_field = self.domain["invoices"]["id_field"]
        unique_id = r[id_field]
        r, status = await self.get("invoices/%s" % unique_id)
        self.assert200(status)
        assert isinstance(r["decimal_number"], str_type)

    async def test_post_decimal_number_fail(self):
        data = {"decimal_number": "100.0.0"}
        r, status = await self.post("/invoices/", data=data)
        self.assert422(status)

    async def test_post_referential_integrity(self):
    ...
    async def test_dbref_post_referential_integrity(self):
    ...
    async def test_post_referential_integrity_list(self):
    ...
    async def test_post_allow_unknown(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        ...
        r = await self.test_client.get("%s/%s" % (self.known_resource_url, id))
        r_data = json.loads(await r.get_data())
        self.assertTrue("unknown" in r_data)
        self.assertEqual("unknown", r_data["unknown"])

    async def test_post_mapping_allow_unknown_allowed(self):
        schema = {
            "data": {
                "type": "dict",
                "allow_unknown": True,
        ...
        await self.app.register_resource("endpoint", settings)

        data = {"data": {"prop": "test prop", "test": "test"}}
        r, status = await self.post("endpoint", data=data)
        self.assert201(status)

    async def test_post_with_content_type_charset(self):
        test_field = "ref"
        test_value = "1234567890123456789054321"
        data = {test_field: test_value}
        r, status = await self.post(
    ...
    async def test_post_with_extra_response_fields(self):
    ...
    async def test_post_with_get_override(self):
        ...
        r = await self.test_client.get(self.known_resource_url, json=data, headers=headers)
        self.assert201(r.status_code)
        self.assertPostResponse(json.loads(await r.get_data()))

    async def test_post_list_of_objectid(self):
    ...
    async def test_post_nested_dict_objectid(self):
        objectid = "50656e4538345b39dd0414f0"
        del self.domain["contacts"]["schema"]["ref"]["required"]
    ...
    async def test_post_valueschema_with_objectid(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        data = {"dict_valueschema": {"id": {"challenge": "50656e4538345b39dd0414f0"}}}
        r, status = await self.post(self.known_resource_url, data=data)
        self.assert201(status)

    async def test_post_list_fixed_len(self):
        objectid = "50656e4538345b39dd0414f0"
        del self.domain["contacts"]["schema"]["ref"]["required"]
    ...
    async def test_custom_issues(self):
    ...
    async def test_custom_etag_update_date(self):
    ...
    async def test_custom_date_updated(self):
    ...
    async def test_subresource(self):
        response, status = await self.post("users/%s/invoices" % self.item_id, data={})
        self.assert201(status)
        self.assertPostResponse(response)

        invoice_id = response.get(self.domain["peopleinvoices"]["id_field"])
        response, status = await self.get("users/%s/invoices/%s" % (self.item_id, invoice_id))
        self.assert200(status)
        self.assertEqual(response.get("person"), self.item_id)

    async def test_subresource_required_ref(self):
        response, status = await self.post(
    ...
    async def test_post_ifmatch_disabled(self):
    ...
    async def test_post_with_relation_to_custom_idfield(self):
        ...
        self.assertTrue(id_field in r)
        self.assertItemLink(r["_links"], r[id_field])
        r, status = await self.get("products", item="BAR")
        self.assertEqual(r["parent_product"], existing_product[id_field])

    async def test_post_bandwidth_saver(self):
    ...
    async def test_post_bandwidth_saver_credit_rule_broken(self):
        data = [
            {
                "amount": 300.0,
                "duration": "months",
        ...
        r, status = await self.post("credit_rules", data=data)
        self.assert201(status)
        self.assertPostResponse(r)
    ...
    async def test_post_alternative_payload(self):
        payl = {"ref": "5432112345678901234567890", "role": ["agent"]}
        async with self.app.test_request_context(self.known_resource_url):
            r, _, _, status, _ = await post(self.known_resource, payl=payl)
        self.assert201(status)
        self.assertPostResponse(r)

    async def test_post_dependency_fields_with_default(self):
    ...
    async def test_post_dependency_required_fields(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        schema = self.domain["contacts"]["schema"]
        schema["dependency_field3"]["required"] = True

        r, status = await self.post(self.known_resource_url, data={})
        self.assertValidationErrorStatus(status)
    ...
    async def test_post_dependency_fields_with_values(self):
        ...
        await self.app.register_resource("posts", settings)

        data = {"field1": "three", "field2": 7}
        r, s = await self.post("posts", data=data)
        self.assert422(s)
    ...
    async def test_post_dependency_fields_with_subdocuments(self):
        ...
        r, s = await self.post("endpoint", data=data)
        self.assert422(s)
    ...
    async def test_post_readonly_field_with_default(self):
    ...
    async def test_post_with_nested_default(self):
    ...
    async def test_post_readonly_in_dict(self):
    ...
    async def test_post_valueschema_dict(self):
    ...
    async def test_post_keyschema_dict(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
    ...
    async def test_post_internal(self):
        # test that post_internal is available and working properly.
        test_field = "ref"
        test_value = "1234567890123456789054321"
        payload = {test_field: test_value}
        async with self.app.test_request_context(self.known_resource_url):
            r, _, _, status, _ = await post_internal(self.known_resource, payl=payload)
        self.assert201(status)

    async def test_post_internal_skip_validation(self):
        ...
        payload = {test_field: test_value}
        async with self.app.test_request_context(self.known_resource_url):
            r, _, _, status, _ = await post_internal(
                self.known_resource, payl=payload, skip_validation=True
            )
        self.assert201(status)

    async def test_post_nested(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
        data = {
        ...
        r, status = await self.post(self.known_resource_url, data=data)
        self.assert201(status)
        values = (await self.compare_post_with_get(
    ...
    async def test_post_error_as_list(self):
        del self.domain["contacts"]["schema"]["ref"]["required"]
    ...
    async def test_id_field_included_with_document(self):
    ...
    async def test_post_type_coercion(self):
        ...
        await self.assertPostItem(data, "aninteger", 42)

    async def test_post_location_header_hateoas_on(self):
        self.app.config["HATEOAS"] = True
        data = {"ref": "1234567890123456789054321"}
        headers = [("Content-Type", "application/json")]
        r = await self.test_client.post(self.known_resource_url, json=data, headers=headers)
        self.assertTrue("Location" in r.headers)
        self.assertTrue(self.known_resource_url in r.headers["Location"])

    async def test_post_location_header_hateoas_off(self):
    ...
    async def test_post_custom_json_content_type(self):
        data = {"ref": "1234567890123456789054321"}
        r, status = await self.post(
    ...
    async def test_post_updating_a_document_with_nullable_data_relation_does_not_fail(self):
        # See #1159.
        del self.domain["contacts"]["schema"]["ref"]["required"]
        ...
        await self.app.register_resource("employee", {"schema": employee})

        data = {"employer": None}
        r, s = await self.post("employee", data=data)
        self.assert201(s)
    ...
    async def test_post_dont_normalize_dotted_fields(self):
        ...
        headers = [("Content-Type", "application/json")]
        resp = await self.test_client.post("test/", json=data, headers=headers)
        _, status = await self.parse_response(resp)
        self.assertTrue(json.loads(await resp.get_data())["a_dict"]["dotted.field"])

    async def test_post_projection_is_honored(self):
    ...
    async def test_unique_within_resource_value_different_resources(self):
        r, status = await self.post("tenant_a", data={"name": "John"})
        self.assert201(status)
        r, status = await self.post("tenant_b", data={"name": "John"})
        self.assert201(status)

    async def test_unique_within_resource_in_resource_without_filter(self):
        def make_payload(unique_value):
            return {"unique_within_resource_attribute": unique_value}

        r, status = await self.post("test_unique", data=make_payload("unique_value"))
        self.assert201(status)
        r, status = await self.post("test_unique", data=make_payload("unique_value"))
        self.assert422(status)
        r, status = await self.post("test_unique", data=make_payload("unique_value_2"))
        self.assert201(status)

    async def test_unique_in_root_attribute(self):
        def make_payload(unique_value):
    ...
    async def test_unique_in_dict_attribute(self):
        def make_payload(unique_value):
    ...
    async def test_unique_in_list_attribute(self):
        def make_payload(unique_value):
    ...
    async def test_unique_in_deep_dict_attribute(self):
        def make_payload(unique_value):
            return {
    ...
    async def test_unique_in_deep_list_attribute(self):
        def make_payload(unique_value):
            return {
    ...
    async def perform_post(self, data, valid_items=[0]):
        r, status = await self.post(self.known_resource_url, data=data)
        self.assert201(status)
        self.assertPostResponse(r, valid_items)
        return r

    async def assertPostItem(self, data, test_field, test_value):
        r = await self.perform_post(data)
        item_id = r[self.domain[self.known_resource]["id_field"]]
        item_etag = r[ETAG]
        db_value = await self.compare_post_with_get(item_id, [test_field, ETAG])
    ...
    async def compare_post_with_get(self, item_id, fields):
        raw_r = await self.test_client.get("%s/%s" % (self.known_resource_url, item_id))
        item, status = await self.parse_response(raw_r)
        id_field = self.domain[self.known_resource]["id_field"]
        self.assert200(status)
...
class TestEvents(TestBase):
    ...
    async def test_on_pre_POST(self):
        devent = DummyEventAsyncIO(self.before_insert)
        self.app.on_pre_POST += devent
        await self.post()
        self.assertFalse(devent.called is None)

    async def test_on_pre_POST_contacts(self):
        devent = DummyEventAsyncIO(self.before_insert)
        self.app.on_pre_POST_contacts += devent
        await self.post()
        self.assertFalse(devent.called is None)

    async def test_on_post_POST(self):
        devent = DummyEventAsyncIO(self.after_insert)
        self.app.on_post_POST += devent
        await self.post()
        self.assertEqual(devent.called[0], self.known_resource)

    async def test_on_POST_post_resource(self):
        devent = DummyEventAsyncIO(self.after_insert)
        self.app.on_post_POST_contacts += devent
        await self.post()
        self.assertFalse(devent.called is None)

    async def test_on_insert(self):
        devent = DummyEventAsyncIO(self.before_insert, True)
    ...
    async def test_on_insert_contacts(self):
        devent = DummyEventAsyncIO(self.before_insert, True)
        self.app.on_insert_contacts += devent
        await self.post()
        self.assertEqual(self.new_contact_id, devent.called[0][0]["ref"])

    async def test_on_inserted(self):
        devent = DummyEventAsyncIO(self.after_insert, True)
        self.app.on_inserted += devent
        await self.post()
        self.assertEqual(self.known_resource, devent.called[0])
        self.assertEqual(self.new_contact_id, devent.called[1][0]["ref"])

    async def test_on_inserted_contacts(self):
        devent = DummyEventAsyncIO(self.after_insert, True)
    ...
    async def post(self):
        headers = [("Content-Type", "application/json")]
        data = {"ref": self.new_contact_id}
        await self.test_client.post(self.known_resource_url, json=data, headers=headers)

    async def before_insert(self):
        db = self.connection[MONGO_DBNAME]
        return await db.contacts.find_one({"ref": self.new_contact_id}) is None

    async def after_insert(self):
        return not await self.before_insert()
